package com.pacifique.todoapp.config.utils.events.listener;

import com.pacifique.todoapp.config.utils.EmailService;
...
public class RegistrationCompleteEventListener
    ...
    public void onApplicationEvent(RegistrationCompleteEvent event) {
        // create the verification Token for user with a verify link
        User user = event.getUser();
        String token = UUID.randomUUID().toString();
        userService.saveVerifyToken(user, token);
        String verificationLink =
            event.getVerifyUrl() + "verifyRegistration?token=" + token;
        ...
        try {
            emailService.sendAccountVerifyEmail(
                recipient,
                subject,
                recipientName,
                verificationLink
